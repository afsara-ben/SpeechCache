...
pwd = os.getcwd()
wav_path = os.path.join(pwd, 'user_study_recordings/')  #aben:user study
# wav_path = os.path.join(pwd, 'user_study_recordings/w:o heaset/'
# create folder first if doesnt exist
if unseen:
    folder_path = os.path.join(pwd, 'models/SLURP/user-study-headset-base-audio-bucket-unseen')
    # folder_path = os.path.join(pwd, 'models/SLURP/user-study-wo-headset-base-audio-bucket-unseen')
else:
    folder_path = os.path.join(pwd, 'models/SLURP/user-study-headset-base-audio-bucket-70%-seen')
    # folder_path = os.path.join(pwd, 'models/SLURP/user-study-wo-headset-base-audio-bucket-70%-seen')
...
def train_test_split(df_):
    df = df_.drop_duplicates('sentence', keep='first')
    df_train = df.sample(frac=0.7, random_state=42)
    df_test = df_[~df_.index.isin(df_train.index)]

    return df_train, df_test

def unseen_train_test_split(df_):
    df = df_.drop_duplicates('sentence', keep='first')
    df_train = df.sample(frac=0.5, random_state=42)
    df_test = df.drop(df_train.index)

    duplicates = df_.loc[df_['sentence'].duplicated(keep='first')]
    # Append duplicates to df_train or df_test based on sentence value
    for idx, row in duplicates.iterrows():
        if row['sentence'] in df_test['sentence'].values:  # as for training only one sample is needed
            df_test = df_test._append(row)

    df_train.reset_index(drop=True, inplace=True)
    df_test.reset_index(drop=True, inplace=True)

    if len(df_train[df_train['sentence'].isin(df_test['sentence'])]):
        print('unseen train test creation ok ')
...
def train(model, optim, df, ctc_loss, bucket):
    if unseen:
        train, test = unseen_train_test_split(df)
    else:
        train, test = train_test_split(df)
    num_nan_train = 0
    transcripts = np.unique(train['sentence'])
    training_idxs = set()
    ...
    print('train %d test %d' % (len(training_idxs), len(test)))

    # filename = f'slurp_unseen_multicache_{user_id}_audio_bucket_{bucket}'
    # filename = f'slurp_curated_headset_base_unseen_{user_id}_audio_bucket_{bucket}'
    if unseen:
        print("print(f'----------------------- unseen% seen benchmark -----------------------')")
        filename = f'user_study_headset_base_unseen_{user_id}_audio_bucket_{bucket}'
        # filename = f'user_study_wo_headset_base_unseen_{user_id}_audio_bucket_{bucket}'
    else:
        print("print(f'----------------------- 70% seen benchmark -----------------------')")
        filename = f'user_study_headset_base_70%_seen_{user_id}_audio_bucket_{bucket}'
        # filename = f'user_study_wo_headset_base_70%_seen_{user_id}_audio_bucket_{bucket}'

    del model.pretrained_model.word_layers
    del model.pretrained_model.word_linear
    del model.intent_layers
    new_file_path = os.path.join(folder_path, filename + '.pth')
...
for _, user_id in tqdm(enumerate(speakers), total=len(speakers)):
    ...
    # filename = f'slurp_unseen_multicache_{user_id}_audio_bucket_3.pkl.gz'
