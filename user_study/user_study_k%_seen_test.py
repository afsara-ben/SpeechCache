...
for _, user_id in tqdm(enumerate(speakers), total=len(speakers)):
    print(f'unseen {unseen} EVAL FOR SPEAKER ', user_id)
    print('count ', slurp_df[slurp_df['user_id'] == user_id].shape[0])
    # filename = f'slurp_70%_multicache_{user_id}_audio_bucket_3.pkl.gz'
    ...
    for bucket in buckets:
        ...
        print(f'bucket {bucket} train {len(train)} test: {len(test)}')
        ...
        if cluster_ids and cluster_centers and phoneme_list:
            total, l1_total, l2_total, l1_hits, l2_hits, l1_correct, l2_correct = multicache_test(model, test, cluster_ids,
                                                                                        cluster_centers,
...
print(f'testing for unseen {unseen}')
...
cumulative_hit_rate, cumulative_cache_acc =0, 0
...
print(f'cumulative_l1_hits {cumulative_l1_hits} cumulative_l1_sample {cumulative_l1_sample}')
print(f'cumulative_l2_hits {cumulative_l2_hits} cumulative_l2_sample {cumulative_l2_sample}')
print(f'cumulative_hits {cumulative_hits} cumulative_sample {cumulative_sample}')
# for i in buckets:
#     print('bucket', i)
#     if bckt[i]['l1']['total']:
