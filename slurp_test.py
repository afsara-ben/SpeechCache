...
if in_domain:
    ...
    cloud_model.load_state_dict(
        torch.load("SLURP/models/slurp-pretrained.pth", map_location=device)) # load trained model
else:
...
def multicache_test(model, df, cluster_ids, cluster_centers, training_idxs, intent_list, L1_THRESHOLD,
                    L2_THRESHOLD):
    # ----------------- prepare for cluster -----------------
...
for _, user_id in tqdm(enumerate(speakers), total=len(speakers)):
    ...
    for bucket in buckets:
        ...
        if not os.path.exists(user_model_dir):
            ...
            break
        file_path = os.path.join(user_model_dir, filename + '.pth')
        model = torch.load(file_path)
        with gzip.open(os.path.join(user_model_dir, filename + '.pkl.gz'), 'rb') as f:
