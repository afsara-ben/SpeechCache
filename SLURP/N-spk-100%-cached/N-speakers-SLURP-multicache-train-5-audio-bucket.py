...
folder_path = os.path.join(pwd, 'models/SLURP/models-N-speakers-SLURP-multicache-five-audio-bucket')
...
def train(speakers, model, optim, train_df, test_df, ctc_loss, bucket):
    ...
    filename = f'3-spk-SLURP-multicache-{speakers}_five_audio_bucket_{bucket}.pkl'
...
for _, speakers in tqdm(enumerate(spk_group), total=len(spk_group)):
    ...
    df1, df2, df3, df4, df5 = pd.DataFrame(columns=slurp_df.columns), pd.DataFrame(
        columns=slurp_df.columns), pd.DataFrame(
        columns=slurp_df.columns), pd.DataFrame(
        columns=slurp_df.columns), pd.DataFrame(
        columns=slurp_df.columns)
    df1_test, df2_test, df3_test, df4_test, df5_test = pd.DataFrame(columns=slurp_df.columns), pd.DataFrame(
        columns=slurp_df.columns), pd.DataFrame(columns=slurp_df.columns), pd.DataFrame(
    ...
    for _, row in test_set.iterrows():
        ...
        if 0 <= (get_audio_duration(wav)) <= 2.7:
            ...
            df4_test = df4_test.append(row)
        else:
            df5_test = df5_test.append(row)
    ...
    train(speakers, model4, optim4, df4, df4_test, ctc_loss4, bucket=4)
    ...
    train(speakers, model5, optim5, df5, df5_test, ctc_loss5, bucket=5)
