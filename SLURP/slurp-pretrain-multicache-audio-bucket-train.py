...
for _, user_id in tqdm(enumerate(speakers), total=len(speakers)):
    ...
    filename1 = f'slurp-pretrain-multicache-audio-bucket-{user_id}-3'
    file_path1 = os.path.join(folder_path, filename1+'.pth')
    file_path2 = os.path.join(folder_path, filename1 + '.pkl.gz')
    ...
    optim1 = torch.optim.Adam(model1.parameters(), lr=1e-4)
    ...
    optim3 = torch.optim.Adam(model3.parameters(), lr=1e-4)
